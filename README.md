...
2. Favorites Management ( Favorites.jsx ) : 
...
1. Component Structure : How to break down a UI into reusable parts ( MovieCard , NavBar ).
2. Data Fetching : How to use useEffect to trigger side effects (like API calls) and async/await with fetch to get data from an external server ( Home.jsx ).
3. State Management :
...
4. User Interaction : Handling events like form submissions (search) and button clicks (toggling favorites).
5. Persistence : Learning how to interact with localStorage to keep data between sessions.
